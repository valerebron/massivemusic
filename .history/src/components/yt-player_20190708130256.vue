<template>
  <main :class="currentState">
    ...
    <section :class="tracksState" class="tracks">
      <input type="search" @input="search(e.target.value)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      playPrev() {
        let prevTrack = document.querySelector('[data-id="'+this.currentYtid+'"]').previousSibling
        if(prevTrack !== null) {
          let prevId = prevTrack.getAttribute('data-id')
          this.play(prevId)
        }
      },
      playNext() {
        let nextTrack = document.querySelector('[data-id="'+this.currentYtid+'"]').nextSibling
        if(nextTrack !== null) {
          let nextId = nextTrack.getAttribute('data-id')
          this.play(nextId)
        }
      },
      playerReady() {
      ...
      search(query) {
        if(query == 'all' || query == '') {
          ...
          this.tracksState = 'loading'
          axios
            .get(window.APIURL+'/tracks/s/'+e.target.value)
            .then((res) => {
              this.tracks = res.data
              this.tracksState = ''
            })
    ...
    mounted: function() {
      this.search(e.target.value = 'all')
    },
  }
</script>
